...
(* abstract syntax and values for \timpcore S381b *)
(* definitions of [[exp]] and [[value]] for \timpcore 340f *)
datatype value = NUM   of int
               | ARRAY of value array
(* definitions of [[exp]] and [[value]] for \timpcore 341a *)
datatype exp   = LITERAL of value
               ...
               (* \timpcore\ syntax for equality and printing 341b *)
               | EQ      of exp * exp
               | PRINTLN of exp
               | PRINT   of exp
               (* array extensions to \timpcore's abstract syntax 353d *)
               | AMAKE of exp * exp
               | AAT   of exp * exp
               | APUT  of exp * exp * exp
               | ASIZE of exp
(* definition of type [[func]], to represent a \timpcore\ function 341e *)
datatype func = USERDEF   of name list * exp
              | PRIMITIVE of value list -> value
(* definition of [[def]] for \timpcore 341c *)
type userfun = { formals : (name * ty) list, body : exp, returns : ty }
datatype def = VAL    of name * exp
             | EXP    of exp
             | DEFINE of name * userfun
(* definition of [[unit_test]] for \timpcore 341d *)
...
(* definition of [[valueString]] for \timpcore S386b *)
...
(* definition of [[expString]] for \timpcore S385b *)
fun expString e =
  let fun bracket s = "(" ^ s ^ ")"
...
(* definitions of [[defString]] and [[defName]] for \timpcore S385c *)
fun defString d =
  let fun bracket s = "(" ^ s ^ ")"
      val bracketSpace = bracket o spaceSep
...
(* definitions of functions [[toArray]] and [[toInt]] for \timpcore 354a *)
...
(* utility functions on \timpcore\ values S383b *)
...
(* type checking for \timpcore 347a *)
fun typeof (e, globals, functions, formals) =
  let (* function [[ty]], checks type of expression, given $\itenvs$ 347b *)
      fun ty (LITERAL v) = INTTY
      (* function [[ty]], checks type of expression, given $\itenvs$ 348a *)
        | ty (VAR x) = (find (x, formals) handle NotFound _ => find (x, globals)
                                                                               )
      (* function [[ty]], checks type of expression, given $\itenvs$ 348b *)
        | ty (SET (x, e)) =
      ...
      (* function [[ty]], checks type of expression, given $\itenvs$ 348d *)
        | ty (IFX (e1, e2, e3)) =
            let val tau1 = ty e1
                val tau2 = ty e2
                val tau3 = ty e3
      ...
      (* function [[ty]], checks type of expression, given $\itenvs$ 349a *)
        | ty (WHILEX (e1, e2)) =
            let val tau1 = ty e1
                val tau2 = ty e2
            in  if eqType (tau1, BOOLTY) then
      ...
      (* function [[ty]], checks type of expression, given $\itenvs$ 349b *)
        | ty (BEGIN es) =
             let val bodytypes = map ty es
             in  List.last bodytypes handle Empty => UNITTY
             end
      (* function [[ty]], checks type of expression, given $\itenvs$ 349c *)
        | ty (EQ (e1, e2)) =
      ...
      (* function [[ty]], checks type of expression, given $\itenvs$ 350a *)
        | ty (PRINTLN e) = (ty e; UNITTY)
        | ty (PRINT   e) = (ty e; UNITTY)
      (* function [[ty]], checks type of expression, given $\itenvs$ 350b *)
        | ty (APPLY (f, actuals)) =
             let val actualtypes                     = map ty actuals
                 val FUNTY (formaltypes, resulttype) = find (f, functions)
                 (* definition of [[badParameter]] S381c *)
...
(* function [[ty]], checks type of expression, given $\itenvs$ ((prototype)) 357 *)
      | ty (AAT (a, i))        =
            (case (ty a, ty i)
                of (ARRAYTY tau1, INTTY) => tau1
                | (_, INTTY)             => raise TypeError
                                            "Cannot access element in nonarray"
                | _                      => raise TypeError
                                                ("Cannot access element" ^
                                                 " at non integer index"))
      | ty (APUT (a, i, e))    =
            (case (ty a, ty i, ty e)
                of (ARRAYTY tau1, INTTY, tau2) =>
                    if eqType (tau1, tau2) then
                        tau1
                    else
                        raise TypeError
                                    "Type mismatch between array and element"
                | (_, INTTY, tau2)             => raise TypeError
                                                    ("Cannot insert " ^
                                                    "element in nonarray")
                | _                            => raise TypeError
      ...
      | ty (AMAKE (len, init)) =
            (case (ty len, ty init)
                of (INTTY, tau1)  => ARRAYTY tau1
              | _                 => raise TypeError ("Length must be an int" ^
                                                     "when creating array"))

      | ty (ASIZE a)           =
            (case ty a
                of (ARRAYTY tau1) => INTTY
                | _               => raise TypeError ("Cannot find size of " ^
                                                       "nonarray"))
...
(* type checking for \timpcore 350c *)
fun typdef (d, globals, functions) =
    case d
      of (* cases for typing definitions in \timpcore 350d *)
           VAL (x, e) =>
             if not (isbound (x, globals)) then
               ...
               in  if eqType (tau, tau') then
                  ...
                  (* raise [[TypeError]] with message about redefinition 351a *)
                     raise TypeError ("Global variable " ^ x ^ " of type " ^
                                                               typeString tau' ^
                                      " may not be redefined with type " ^
                                                                 typeString tau)
               end
         (* cases for typing definitions in \timpcore 351b *)
         | EXP e =>
             let val tau = typeof (e, globals, functions, emptyEnv)
             in  (globals, functions, typeString tau)
             end
         (* cases for typing definitions in \timpcore 351c *)
...
(* lexical analysis and parsing for \timpcore, providing [[filexdefs]] and [[stringsxdefs]] S386c *)
...
(* parsers and [[xdef]] streams for \timpcore S386d *)
...
(* parsers and [[xdef]] streams for \timpcore S387b *)
...
(* parsers and [[xdef]] streams for \timpcore S387c *)
...
(* parsers and [[xdef]] streams for \timpcore S387d *)
...
(* parsers and [[xdef]] streams for \timpcore S387e *)
val xdeftable = usageParsers
  [ ("(use filename)", USE <$> name)
  (* rows added to \timpcore\ [[xdeftable]] in exercises S388b *)
...
val xdef =  DEF  <$> deftable
        ...
        <|> DEF <$> EXP <$> exp
        <?> "definition"
(* parsers and [[xdef]] streams for \timpcore S388a *)
...
(* evaluation, testing, and the read-eval-print loop for \timpcore S388c *)
(* definitions of [[eval]] and [[evaldef]] for \timpcore S388d *)
val unitVal = NUM 1983
(* definitions of [[eval]] and [[evaldef]] for \timpcore S388e *)
...
fun eval (e, globals, functions, formals) =
  ...
  let val toBool = projectBool
      ...
      fun ev (LITERAL n)          = n
        ...
        | ev (APPLY (f, args)) =
            (case find (f, functions)
               of PRIMITIVE p  => p (map ev args)
                | USERDEF func =>
                    (* apply user-defined function [[func]] to [[args]] S389a *)
        ...
        (* more alternatives for [[ev]] for \timpcore 354b *)
        | ev (AAT (a, i)) =
...
(* definitions of [[eval]] and [[evaldef]] for \timpcore S389b *)
...
val _ = op evaldef : def * value ref env * func env -> value ref env * func env
                                                                        * string
(* definitions of [[basis]] and [[processDef]] for \timpcore S382a *)
...
(* definition of [[testIsGood]] for \timpcore S383c *)
...
(* implementations of \timpcore\ primitives and definition of [[initialBasis]] S382d *)
...
(* utility functions and types for making \timpcore\ primitives S389f *)
val arithtype = FUNTY ([INTTY, INTTY], INTTY)
(* type declarations for consistency checking *)
val _ = op arithtype : funty
(* utility functions and types for making \timpcore\ primitives S390c *)
...
val initialBasis =
  let fun addPrim ((name, prim, funty), (tfuns, vfuns)) =
      ...
      val (tfuns, vfuns)  = foldl addPrim (emptyEnv, emptyEnv)
                            (
                           (* primitive functions for \timpcore\ [[::]] S390a *)
                             ("+", arithOp op +,   arithtype) ::
                             ("-", arithOp op -,   arithtype) ::
                             ("*", arithOp op *,   arithtype) ::
                             ("/", arithOp op div, arithtype) ::

                           (* primitive functions for \timpcore\ [[::]] S390b *)
