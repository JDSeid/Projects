...
fun atLoc loc f a =
  f a handle e as RuntimeError _ => raise Located (loc, e)
           ...
           (* more handlers for [[atLoc]] ((type-checking)) S382c *)
...
(* sets of free type variables in \tuscheme 381a *)
fun freetyvars t =
...
(* sets of free type variables in \tuscheme 381b *)
...
(* shared utility functions on sets of type variables 408 *)
...
(* kind checking for {\tuscheme} 387b *)
...
(* kind checking for {\tuscheme} 389b *)
fun asType (ty, Delta) =
...
(* abstract syntax and values for {\tuscheme} S393b *)
(* definitions of [[exp]] and [[value]] for {\tuscheme} 370a *)
...
(* definitions of [[exp]] and [[value]] for {\tuscheme} 370b *)
and    value = NIL
             | BOOLV     of bool
             | NUM       of int
             | SYM       of name
...
(* definition of [[def]] for {\tuscheme} 370c *)
...
val _ = op valueString : value -> string
  ...
  | valueString (ARRAY vs) =
      ...
      "[" ^ separate ("", " ") (map valueString (Array.foldr op :: [] vs)) ^ "]"
                                                                        (*OMIT*)
(* definition of [[expString]] for {\tuscheme} S402b *)
fun expString e =
  let fun bracket s = "(" ^ s ^ ")"
...
(* definitions of [[defString]] and [[defName]] for {\tuscheme} S403 *)
fun defString d =
  let fun bracket s = "(" ^ s ^ ")"
      val bracketSpace = bracket o spaceSep
...
(* capture-avoiding substitution for {\tuscheme} 384a *)
fun tysubst (tau, varenv) =
  let
  (* definition of [[renameForallAvoiding]] for {\tuscheme} ((prototype)) 407 *)
...
(* capture-avoiding substitution for {\tuscheme} 385a *)
...
(* capture-avoiding substitution for {\tuscheme} 385b *)
...
(* type equivalence for {\tuscheme} 379a *)
...
(* Boolean saying if \monobox{FORALL (alphas, tau)} $\equiv$ \monobox{FORALL (alphas', tau')} 379b *)
      let fun ok a  =
...
fun typeof (e, kindenv, typeenv) =
    (* ty (exp) Takes in an expression, returns a tyex representing the
        type of the expression. *)
    let fun ty (LITERAL v)                   =
        (* literal (e) Takes in a literal expression. Returns the type of the
           literal *)
        let fun literal (NUM n)              = inttype
              | literal (BOOLV b)            = booltype
              | literal (SYM s)              = symtype
              | literal (NIL)                = FORALL(["'a"], listtype tvA)
              | literal (PAIR (v1, NIL))     = listtype (ty (LITERAL v1))
              | literal (PAIR (v1, v2))      =
                    let val tau1 = ty (LITERAL v1)
                        val tau2 = ty (LITERAL v2)
                    in
                        if eqType (listtype tau1, tau2) then
                            tau2
                        else
                            raise TypeError "List must have only one type"
                    end
              | literal _                    = raise TypeError "invalid literal"
        in
            literal v
        end
          ...
          | ty (IFX (e1, e2, e3))             =
                (case (ty e1, ty e2, ty e3)
                    of (tau1, tau2, tau3) =>
                        if eqType(tau1, booltype) andalso
                            eqType(tau2, tau3) then
                            tau2
                        else
                            raise TypeError "Invalid if")
          | ty (VAR n)                        = find (n, typeenv)

          | ty (APPLY (e, es))                =
                (case (ty e, List.map ty es)
                    of (FUNTY (argTypes, returnType), taus) =>
                        if eqTypes(argTypes, taus) then
                            returnType
                        else
                            raise TypeError "Invalid apply"
                    |  _                                    =>
                                    raise TypeError "Can't apply nonfunction")

          | ty (LETX (LET, nes, e))           =
                let val names           = List.map fst nes
                    val types           = List.map (fn (ne) => ty (snd ne)) nes
                    val new_typeenv    = bindList (names, types, typeenv)
                in
                    typeof (e, kindenv, new_typeenv)
                end
                
          | ty (LAMBDA (nts, e))              =
                (* asTypePair (nt) takes in a name type pair. Calls asType on
                    the type and returns the result *)
                let fun asTypePair (nt) = asType (snd nt, kindenv)
                    val typePairs       = List.map asTypePair nts
                    val nameList        = List.map fst nts
                    val new_typeenv     = bindList (nameList, typePairs,
                                                                        typeenv)
                in
                    FUNTY(typePairs, typeof (e, kindenv, new_typeenv))
                end
                ...
                (case (ty e1, ty e2)
                    of (tau1, tau2) =>
                        if eqType(tau1, booltype) then
                            ...
                            raise TypeError "e1 must be a bool")

          | ty (BEGIN es)                     = List.last (List.map ty es)

          | ty (SET (n, e))                   =
                if eqType (find (n, typeenv), ty e) then
                    ty e
                else
          ...
          | ty (LETX (LETSTAR, ne::nes, e))   =
                ty (LETX (LET, [ne], LETX(LETSTAR, nes, e)))

          | ty (LETRECX (ntes, e))            =
                (* getAsType (nte) Gets the type from a name * type * expression
                   tuple, and applies asType to it *)
                let fun getAsType (nte)  = asType (snd (fst nte), kindenv)
                    val types            = List.map getAsType ntes
                    val names            = List.map (fn (nte) =>
                                                        fst (fst nte)) ntes
                    val new_env          = bindList (names, types, typeenv)
                    val exps             = List.map snd ntes
                    val types_of_es      = List.map (fn (en) => typeof (en,
                                                        kindenv, new_env)) exps
                    (* checkLambda (ex) takes in an expression and returns true
                        iff the expression is of the LAMBDA form *)
                    fun checkLambda (ex) =
                        (case ex
                            of (LAMBDA(nes, body)) => true
                             | _                   =>
                                    raise TypeError  ("Letrec variable " ^
                                                      "bound to nonlambda"))
                    val isLambda         = List.map checkLambda exps
                in
                    if eqTypes (types, types_of_es) then
                        typeof (e, kindenv, new_env)
                    else
                        raise TypeError ("Declared type is mismatched" ^
                                                "with actual type")
                end

          | ty (TYLAMBDA (names, e))     =
                let val typeList                 = List.map (fn (n) => TYPE)
                                                                           names
                    val freeVarsList             = freetyvarsGamma typeenv
                    (* hasName (name, freeList) Takes in a name and a list of
                        names. Returns true iff the name is present in
                        the list*)
                    fun hasName (name, freeList) =
                                 List.exists (fn (e1) => e1 = name) freeList
                    val hasFree                  = List.exists (fn (name) =>
                                                   hasName (name, freeVarsList))
                                                                        names
                    val new_kindenv             = bindList (names, typeList,
                                                                        kindenv)
                in
                    if not hasFree then
                        FORALL (names, typeof (e, new_kindenv, typeenv))
                    else
                        raise TypeError "Error: free variables"

                end

          | ty (TYAPPLY (e, taus))               =
                instantiate (ty e, taus, kindenv)
    in
        ty e
    end
(* typdef (definition, kindenv, typeenv) Updates the typeenv to reflect new
   definitions. Returns the new type environment and type of the definition
   as a string *)
fun typdef (definition, kindenv, typeenv) =
      (* tyd (def) Takes in a definition. It updates the type environment, and
      returns the new type environment and the type of the definition as a
      string *)
      let fun tyd (VAL (n, e))            =
        let val typeE = typeof (e, kindenv, typeenv)
        in
                (bind (n, typeE, typeenv), typeString typeE)
        end

      | tyd (VALREC (n, te, e))           =
            (case (n, te, e)
                of (name, tau, LAMBDA(nes, body)) =>

                let val asT      = asType (tau, kindenv)
                    val new_env  =  bind (name, asT, typeenv)
                in
                    if eqType (typeof (e, kindenv, new_env), asT) then
                        (new_env, typeString (asType (tau, kindenv)))
                    else
                        raise TypeError "Valrec type mismatch"
                end
                |  _                              => raise TypeError
                                                            "Valrec type error")

      | tyd (EXP e)                       = (typeenv, typeString
                                                 (typeof (e, kindenv, typeenv)))

      | tyd (DEFINE (n, te, (nts, e)))    =
            let val fun_type = FUNTY(List.map snd nts, te)
            in
                tyd (VALREC(n, fun_type, LAMBDA(nts, e)))
            end
    in
        tyd definition
    end
...
(* lexical analysis and parsing for \tuscheme, providing [[filexdefs]] and [[stringsxdefs]] S395a *)
...
(* parsers and [[xdef]] streams for {\tuscheme} S395c *)
fun keyword words =
...
(* parsers and [[xdef]] streams for {\tuscheme} S396a *)
...
(* parsers and [[xdef]] streams for {\tuscheme} S396b *)
...
(* parsers and [[xdef]] streams for {\tuscheme} S397a *)
...
(* parsers and [[xdef]] streams for {\tuscheme} S397b *)
val unit_test =
...
(* parsers and [[xdef]] streams for {\tuscheme} S397c *)
val xdef =
     DEF <$> def
 ...
 <|> DEF <$> EXP <$> exp
 <?> "definition"
(* parsers and [[xdef]] streams for {\tuscheme} S397d *)
...
(* evaluation, testing, and the read-eval-print loop for {\tuscheme} S397e *)
...
(* definitions of [[eval]] and [[evaldef]] for {\tuscheme} S398a *)
fun eval (e, rho) =
  let val go = applyCheckingOverflow id in go end (* OMIT *)
  let fun ev (LITERAL n) = n
        (* alternatives for [[ev]] for [[TYAPPLY]] and [[TYLAMBDA]] 389c *)
        | ev (TYAPPLY  (e, _)) = ev e
        | ev (TYLAMBDA (_, e)) = ev e
        (* more alternatives for [[ev]] for {\tuscheme} S398b *)
        | ev (VAR v) = !(find (v, rho))
        | ev (SET (n, e)) =
            let val v = ev e
            in  find (n, rho) := v;
        ...
        (* more alternatives for [[ev]] for {\tuscheme} S398c *)
        ...
        (* more alternatives for [[ev]] for {\tuscheme} S398d *)
        | ev (LAMBDA (args, body)) = CLOSURE ((map (fn (x, ty) => x) args, body)
                                                                          , rho)
        (* more alternatives for [[ev]] for {\tuscheme} S398e *)
        | ev (APPLY (f, args))  =
               (case ev f
                  of PRIMITIVE prim => prim (map ev args)
                   | CLOSURE clo =>
                       (* apply closure [[clo]] to [[args]] ((mlscheme)) 317b *)
        ...
        (* more alternatives for [[ev]] for {\tuscheme} S398f *)
        | ev (LETX (LET, bs, body)) =
        ...
        (* more alternatives for [[ev]] for {\tuscheme} S399a *)
        | ev (LETRECX (bs, body)) =
...
(* definitions of [[eval]] and [[evaldef]] for {\tuscheme} S399b *)
...
(* definitions of [[basis]] and [[processDef]] for {\tuscheme} S393d *)
(* definition of [[basis]] for {\tuscheme} 391d *)
...
(* other handlers that catch non-fatal exceptions and pass messages to [[caught]] ((type-checking)) S382b *)
...
(* definition of [[testIsGood]] for {\tuscheme} S401e *)
fun testIsGood (test, (Delta, Gamma, rho)) =
 ...
 (* shared [[check{Expect,Assert,Error,Type}Checks]], which call [[ty]] S385a *)
 ...
 (* shared [[check{Expect,Assert,Error,Type}Checks]], which call [[ty]] S402a *)
...
(* shared [[checkTypePasses]] and [[checkTypeErrorPasses]], which call [[ty]] S384c *)
...
(* implementations of \tuscheme\ primitives and definition of [[initialBasis]] S394a *)
...
(* shared utility functions for building primitives in languages with type checking S625e *)
...
(* utility functions and types for making \tuscheme\ primitives 391a *)
val arithtype =
...
(* utility functions and types for making \tuscheme\ primitives S400a *)
...
(* definition of [[primBasis]] for \tuscheme 391e *)
val primBasis =
  let fun addKind ((name, kind), kinds) =
      ...
      val (types, values) = foldl addPrim (emptyEnv, emptyEnv)
                            (
                            (* primitive functions for \tuscheme\ [[::]] 391b *)
                             ("+", arithOp op +,   arithtype) ::
                             ("-", arithOp op -,   arithtype) ::
                             ("*", arithOp op *,   arithtype) ::
                             ("/", arithOp op div, arithtype) ::

                            (* primitive functions for \tuscheme\ [[::]] 391c *)
                           ...
                           (* primitive functions for \tuscheme\ [[::]] S400c *)
      ...
      val (types, values) =
        ...
        (* primitives that aren't functions, for \tuscheme\ [[::]] S400d *)
...
val initialBasis =
  (* let val fundefs   = *)
  let val predefined_included = true
    val fundefs = if not predefined_included then [] else
                       [ ";  predefined {\\tuscheme} functions 368a "
                       , "(val list1 (type-lambda ['a] (lambda ([x : 'a])"
                       ,
                  "                               ([@ cons 'a] x [@ '() 'a]))))"
                       , ";  predefined {\\tuscheme} functions 368b "
                       ,
          ...
          "                               ([@ cons 'a] x ([@ list2 'a] y z)))))"
                       , ";  predefined {\\tuscheme} functions 368c "
                      ...
                      "      (lambda ([x : 'a]) (lambda ([y : 'b]) (f x y))))))"
                       ...
                       , ";  predefined {\\tuscheme} functions 368d "
                       , "(val length"
                       , "  (type-lambda ['a]"
                       , "    (letrec"
                     ...
                     "                  (+ 1 (length-mono ([@ cdr 'a] xs))))))]"
                       , "       length-mono)))"
                       , ";  predefined {\\tuscheme} functions 369b "
                       , "(val revapp"
                       , "  (type-lambda ['a]"
                       ,
       ...
       "                                  ([@ cons 'a] ([@ car 'a] xs) ys)))))]"
                       , "       revapp-mono)))"
                       , ";  predefined {\\tuscheme} functions 369a "
                       , "(val filter"
                       , "  (type-lambda ('a)"
                       , "    (letrec"
                       ,
           ...
           "                                  (filter-mono p? ([@ cdr 'a] xs)))"
              ...
              "                               (map-mono f ([@ cdr 'a] xs))))))]"
                       , "      map-mono)))"
                       , ";  predefined {\\tuscheme} functions S400e "
                       , "(val caar"
                       , "   (type-lambda ('a)"
                       ...
                       , ";  predefined {\\tuscheme} functions S400f "
                       ,
                       ...
                       , ";  predefined {\\tuscheme} functions S401a "
                       , "(val append"
                       , "  (type-lambda ('a)"
                       ,
