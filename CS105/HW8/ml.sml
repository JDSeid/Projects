...
fun solve TRIVIAL = idsubst
  ...
  (* All other cases are ty ~ ty *)
  | solve (ty1 ~ ty2) =
        (case (ty1, ty2)
            of (TYVAR a, t) =>
                let val freeVars = freetyvars t
                in
                    if not (List.exists (fn (x) => x = a) freeVars) then
                        bind (a, t, idsubst)
                    else if eqType (TYVAR a, t) then
                        idsubst
                    else
                        unsatisfiableEquality(ty1, ty2)
                end

            |  (t, TYVAR a) => solve (ty2 ~ ty1)
            |  (TYCON con1, TYCON con2) =>
                if con1 = con2 then
                    idsubst
                else
                    unsatisfiableEquality(ty1, ty2)
            |  (CONAPP (t1, t1s), CONAPP(t2, t2s)) =>
                ...
                solve (ListPair.foldrEq (fn (curr1, curr2, init) => (curr1 ~ curr2) /\ init) (t1 ~ t2) (t1s, t2s))

            | _ => unsatisfiableEquality(ty1, ty2))
...
(* TYCON with TYCON *)
        ...
        val () = Unit.checkAssert "bool ~ bool can be solved"
                  (fn () => hasGoodSolution (booltype ~ booltype))
        val () = Unit.checkAssert "bool ~ bool is solved by the identity subst"
                  (fn () => solutionEquivalentTo (booltype ~ booltype, idsubst))

(* Conjunction case *)
        val () = Unit.checkAssert "TRIVIAL and TRIVIAL can be solved"
                  (fn () => hasGoodSolution (TRIVIAL /\ TRIVIAL))
        val () = Unit.checkAssert "TRIVIAL and TRIVIAL solved by idsubst"
                  (fn () => solutionEquivalentTo (TRIVIAL /\ TRIVIAL, idsubst))
        val () = Unit.checkAssert ("inttype ~ a and b ~ booltype solved by" ^
                                    "[(a -> inttype), (b -> booltype)]")
                  (fn () => solutionEquivalentTo (inttype ~ alpha /\ beta ~
                                                  booltype, [("a", inttype),
                                                  ("b", booltype)]))
        val () = Unit.checkAssert "a ~ bool and a ~ int cannot be solved"
                  (fn () => hasNoSolution (alpha ~ booltype /\ alpha ~ inttype))
        val () = Unit.checkAssert "TRIVIAL and inttype ~ booltype solved"
                  (fn () => solutionEquivalentTo (TRIVIAL /\ booltype ~
                                                             booltype, idsubst))

(* TYVAR with TYCON *)
        val () = Unit.checkAssert "bool ~ a can be solved"
                (fn () => solutionEquivalentTo (booltype ~ alpha,
                                                            [("a", booltype)]))
        val () = Unit.checkAssert "int ~ a can be solved"
                (fn () => hasGoodSolution (inttype ~ alpha))
        val () = Unit.checkAssert "a ~ bool can be solved"
                (fn () => solutionEquivalentTo (alpha ~ booltype,
                                                        [("a", booltype)]))

        val () = Unit.checkAssert "a ~ a can be solved with the idsubst"
                  (fn () => solutionEquivalentTo (alpha ~ alpha, idsubst))



(* CONAPP with CONAPP *)
        val () = Unit.checkAssert "int list with int list should be solvable"
                (fn () => hasGoodSolution (CONAPP(TYCON "list", [inttype]) ~
                                            CONAPP(TYCON "list", [inttype])))
        val () = Unit.checkAssert "intlist should not be solvable with boollist"
                (fn () => hasNoSolution (CONAPP(TYCON "list", [booltype]) ~
                                             CONAPP(TYCON "list", [inttype])))
        val () = Unit.checkAssert ("'a list should be solvable with intlist " ^
                                    "with substitution 'a->int")
                (fn () => solutionEquivalentTo (CONAPP(TYCON "list", [alpha]) ~
              CONAPP(TYCON "list", [inttype]), bind ("a", inttype, idsubst)))
        val () = Unit.checkAssert ("pair of ('a, int) (bool, 'a) should have" ^
                                                                "no solution")
                (fn () => hasNoSolution (CONAPP(TYCON "pair", [alpha, inttype])
                                    ~ CONAPP(TYCON "pair", [booltype, alpha])))
(* TYVAR with CONAPP *)
        val () = Unit.checkAssert "a ~ a list cannot be solved"
                (fn () => hasNoSolution (alpha ~ listtype alpha))
        val () = Unit.checkAssert "a ~ b list can be solved"
                (fn () => hasGoodSolution (alpha ~ listtype beta))
        val () = Unit.checkAssert "a ~ (int, bool) list can be solved"
                (fn () => solutionEquivalentTo (alpha ~ pairtype (inttype,
                                                                      booltype),
                                                 [("a", pairtype (inttype,
                                                    booltype))]))

(* TYCON with CONAPP (wildcard case) *)
        val () = Unit.checkAssert "int ~ a list cannot be solved"
                (fn () => hasNoSolution (inttype ~ listtype alpha))
        val () = Unit.checkAssert "int ~ (int, int) cannot be solved"
                (fn () => hasNoSolution (inttype ~ pairtype(inttype, inttype)))
...
(* function [[literal]], to infer the type of a literal constant ((prototype)) 448e *)
      fun literal (SYM n) = (symtype, TRIVIAL)
        | literal (NUM x) = (inttype, TRIVIAL)
        | literal (BOOLV b) = (booltype, TRIVIAL)
        | literal (NIL) = (listtype alpha, TRIVIAL)
        | literal (PAIR (v, vs)) =
            let val (t1, theta1) = literal v
                val (t2, theta2) =
                    (case vs of NIL => (listtype t1, TRIVIAL)
                             | _   => literal vs)
                in
                    if eqType(listtype t1, t2) then
                        (t2, theta1 /\ theta2)
                    else
                        raise TypeError "Cannot have list with mulitple types"

                end

        | literal _ = raise BugInTypeInference "invalid literal"
      ...
      fun ty (LITERAL n) = literal n
        ...
        | ty (IFX (e1, e2, e3))        =
            let val (tau1, c1) = ty e1
                val (tau2, c2) = ty e2
                val (tau3, c3) = ty e3
                val c = (c1 /\ c2 /\ c3 /\ tau1 ~ booltype /\ tau2 ~ tau3)
            in
                (tau2, c)
        ...
        | ty (BEGIN es)                =
            let val (taus, c) = typesof(es, Gamma)
            in
                ((List.last taus), c)
            end
        | ty (LAMBDA (formals, body))  =
            let val newGamma  = (List.foldl (fn (formal, gamma) => bindtyscheme (formal, FORALL([], freshtyvar()), gamma)) Gamma formals)
                val (tau, c) = (typeof (body, newGamma))
                val aList       = List.map (fn (formal) => freshInstance (findtyscheme (formal, newGamma))) formals
            in
                (funtype (aList, tau), c)
            end
        | ty (LETX (LET, bs, body))    =
            let val es                 = List.map snd bs
                val names              = List.map fst bs
                val (taus, c)          = typesof(es, Gamma)
                val theta              =
                    if (isIdempotent (solve c)) then
                        solve c
                    else
                        raise TypeError "Non-idempotent solution"
                val aList                 = inter ((dom theta), (freetyvarsGamma(Gamma)))
                val cs                 = List.map (fn (a) => (TYVAR a) ~ (varsubst theta a)) aList
                val c_prime            = conjoinConstraints cs
                val as_free            = union ((freetyvarsGamma Gamma), freetyvarsConstraint c_prime)
                val sigmas             = List.map (fn (tau) => generalize ((tysubst theta tau), as_free)) taus
                val newGamma           = ListPair.foldlEq (fn (n, s, gamma) => bindtyscheme (n, s, gamma)) Gamma (names, sigmas)
                val (tau, c_body)      = typeof (body, newGamma)
            in
                (tau, (c_body /\ c_prime))
            end
