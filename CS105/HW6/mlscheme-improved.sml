...
(*****************************************************************)
(*                                                               *)
(*   \FOOTNOTESIZE SHARED: NAMES, ENVIRONMENTS, STRINGS, ERRORS, PRINTING,
 INTERACTION, STREAMS, \&\ INITIALIZATION *)
(*                                                               *)
(*****************************************************************)

(* \footnotesize shared: names, environments, strings, errors, printing,
interaction, streams, \&\ initialization S237a *)
(* for working with curried functions: [[id]], [[fst]], [[snd]], [[pair]],
[[curry]], and [[curry3]] S263d *)
fun id x = x
fun fst (x, y) = x
fun snd (x, y) = y
fun pair x y = (x, y)
...
(* common parsing code ((elided)) (THIS CAN'T HAPPEN -- claimed code was not
used) *)
...
(* abstract syntax and values for \uscheme S365c *)
(* definitions of [[exp]] and [[value]] for \uscheme 313a *)
...
(* definition of [[def]] for \uscheme 313b *)
...
(* definition of [[expString]] for \uscheme S378c *)
fun expString e =
  let fun bracket s = "(" ^ s ^ ")"
...
(*****************************************************************)
(*                                                               *)
(*   LEXICAL ANALYSIS AND PARSING FOR \USCHEME, PROVIDING [[FILEXDEFS]] AND
 [[STRINGSXDEFS]] *)
(*                                                               *)
(*****************************************************************)
...
(* parsers and [[xdef]] streams for \uscheme S376b *)
fun exptable exp =
...
(* parsers and [[xdef]] streams for \uscheme S377a *)
...
(* parsers and [[xdef]] streams for \uscheme S377b *)
...
(* parsers and [[xdef]] streams for \uscheme S377c *)
...
(* parsers and [[xdef]] streams for \uscheme S377e *)
...
(* parsers and [[xdef]] streams for \uscheme S377f *)
val xdefstream =
...
(* evaluation, testing, and the read-eval-print loop for \uscheme S369b *)


(* freeIn : exp -> name -> bool
    Given a uScheme expression and a variable name, returns true iff the name
    represents a free variable in the expression.
 *)


exception LeftAsExercise

(* Takes in an expression and a name. Returns true iff the name is free
    in the expression*)
 fun freeIn exp name =
    let fun free  (LITERAL v)                         = false
          | free (VAR n)                              = n = name
          | free (SET (n, e))                         =
                (freeIn e name) orelse (n = name)
          | free (IFX (e1, e2, e3))                   =
                (freeIn e1 name) orelse (freeIn e2 name) orelse (freeIn e3 name)
          | free (WHILEX (e1, e2))                    =
                (freeIn e1 name) orelse (freeIn e2 name)
          | free (BEGIN es)                           =
                List.exists (fn (x) => freeIn x name) es
          | free (APPLY (e1, es))                     =
                (freeIn e1 name) orelse (
                List.exists (fn (x) => freeIn x name) es)
          | free (LETX (LET, NEs, e)) =
                List.exists (fn (x) => freeIn (snd x) name) NEs orelse
                ((freeIn e name) andalso (not (member name (map fst NEs))))
          | free (LETX (LETREC, NE::NEs, e)) =
                (not (member name (map fst (NE::NEs)))) andalso
                (List.exists (fn (x) => freeIn (snd x) name) (NE::NEs) orelse
                (freeIn e name))
          |  free (LETX (LETSTAR, [],    e))       = freeIn e name
          |  free (LETX (LETSTAR, b::bs, e))      =
                freeIn (LETX (LET, [b], LETX (LETSTAR, bs, e))) name
          | free (LAMBDA (args, e))                   =
                (freeIn e name) andalso
                (not (member name args))
          | free _                                    = false
    in
        free exp
    end
...
(* Unit Testing *)
        ...
        local
          ...
          val set_x2 = SET ("x2", VAR"x")
          val apply1 = APPLY (VAR "fun1", [VAR "x", VAR "y"])
          val lambda_x = LAMBDA(["x"], LAMBDA(["y"],
                                      APPLY (VAR "equal?", [VAR "x", VAR "y"])))
          val lambda_y = LAMBDA (["y"],
                                      APPLY (VAR "equal?", [VAR "x", VAR "y"]))
        in
        val () =
            Unit.checkExpectWith Unit.boolString "freeIn (set literal) true"
            (fn () => freeIn set_x1 "x")
            true
        val () =
            Unit.checkExpectWith Unit.boolString "freeIn (set literal) false"
            (fn () => freeIn set_x1 "y")
            false
        val () =
            Unit.checkExpectWith Unit.boolString
            "freeIn (set var) var being set"
            (fn () => freeIn set_x2 "x2")
            true
        val () =
            Unit.checkExpectWith Unit.boolString
            "freeIn (set var) var in expression"
            (fn () => freeIn set_x2 "x")
            true
        val () =
            Unit.checkExpectWith Unit.boolString "freeIn (set var) false"
            (fn () => freeIn set_x2 "y")
            false
        val () =
            Unit.checkExpectWith Unit.boolString "freeIn apply1 true"
            (fn () => freeIn apply1 "fun1")
            true
        ...
        val () =
            Unit.checkExpectWith Unit.boolString "freeIn lambda_x true"
            (fn () => freeIn lambda_x "equal?")
            true
        val () =
            Unit.checkExpectWith Unit.boolString "freeIn lambda_x false"
            (fn () => freeIn lambda_x "y")
            false
        val () =
            Unit.checkExpectWith Unit.boolString "freeIn lambda_y true"
            (fn () => freeIn lambda_y "x")
            true
        val () =
            Unit.checkExpectWith Unit.boolString "freeIn lambda_y false"
            (fn () => freeIn lambda_y "y")
            false
...
fun improve ((names, e), binds) =
            ((names, e),  List.filter (fn (x) => freeIn (LAMBDA (names, e))
                                                                (fst x)) binds)
...
(* definitions of [[eval]], [[evaldef]], [[basis]], and [[processDef]] for
\uscheme ((elided)) (THIS CAN'T HAPPEN -- claimed code was not used) *)
(* definitions of [[eval]] and [[evaldef]] for \uscheme 316a *)
fun eval (e, rho) =
  let val go = applyCheckingOverflow id in go end (* OMIT *)
  let fun ev (LITERAL v) = v
        (* more alternatives for [[ev]] for \uscheme 316b *)
        | ev (VAR x) = !(find (x, rho))
        | ev (SET (x, e)) =
            let val v = ev e
            in  find (x, rho) := v;
        ...
        (* more alternatives for [[ev]] for \uscheme 316c *)
        ...
        | ev (WHILEX (guard, body)) =
            if projectBool (ev guard) then
              ...
              BOOLV false
        (* more alternatives for [[ev]] for \uscheme 316d *)
        | ev (BEGIN es) =
        ...
        (* more alternatives for [[ev]] for \uscheme 317a *)
        ...
        (* more alternatives for [[ev]] for \uscheme 317c *)
        | ev (LETX (LET, bs, body)) =
        ...
        (* more alternatives for [[ev]] for \uscheme 317d *)
        | ev (LETX (LETSTAR, bs, body)) =
            let fun step ((n, e), rho) = bind (n, ref (eval (e, rho)), rho)
            in  eval (body, foldl step rho bs)
            end
        (* more alternatives for [[ev]] for \uscheme 318a *)
        | ev (LETX (LETREC, bs, body)) =
            let val (names, values) = ListPair.unzip bs

(* if any expression in [[values]] is not a [[lambda]],
reject the [[letrec]] S367a *)
...
(* definitions of [[eval]] and [[evaldef]] for \uscheme 318b *)
...
(* definitions of [[eval]] and [[evaldef]] for \uscheme 318c *)
...
(* definitions of [[eval]] and [[evaldef]] for \uscheme 319 *)
  | evaldef (EXP e, rho) =
...
(* definitions of [[eval]], [[evaldef]], [[basis]],
and [[processDef]] for \uscheme S370c *)
...
(* other handlers that catch non-fatal exceptions and pass messages to
[[caught]] S371b *)
...
(* definition of [[testIsGood]] for \uscheme S378a *)
...
(* definition of [[processXDef]], which can modify [[unitTests]] and call
[[errmsg]] S370b *)
...
(* implementations of \uscheme\ primitives and definition of [[initialBasis]]
S372a *)
(* utility functions for building primitives in \uscheme 320a *)
fun inExp f =
  fn (e, vs) => f vs
...
(* utility functions for building primitives in \uscheme 320b *)
...
(* utility functions for building primitives in \uscheme 320c *)
fun arithOp f = binaryOp (fn (NUM n1, NUM n2) => NUM (f (n1, n2))
                           | (NUM n, v) =>
                                       (* report [[v]] is not an integer 321b *)
                                           raise RuntimeError (
...
(* utility functions for building primitives in \uscheme 321a *)
...
val initialBasis =
  let val rho =
        foldl (fn ((name, prim), rho) => bind (name, ref (PRIMITIVE (inExp prim)
              ...
              emptyEnv ((* primitives for \uscheme\ [[::]] 320d *)
                        ("+", arithOp op +  ) ::
                        ("-", arithOp op -  ) ::
                        ("*", arithOp op *  ) ::
                        ("/", arithOp op div) ::
                        (* primitives for \uscheme\ [[::]] 321c *)
                        ("<", intcompare op <) ::
                        (">", intcompare op >) ::
      ...
      val fundefs =
                     [ ";  predefined uScheme functions 98a "
                     , "(define caar (xs) (car (car xs)))"
                     , "(define cadr (xs) (car (cdr xs)))"
                     , "(define cdar (xs) (cdr (car xs)))"
                     ,
                 ";  more predefined combinations of [[car]] and [[cdr]] S311c "
                     ...
                     ,
                 ";  more predefined combinations of [[car]] and [[cdr]] S311d "
                     ...
                     ,
                 ";  more predefined combinations of [[car]] and [[cdr]] S311e "
                     ...
                     , ";  predefined uScheme functions 98b "
                     , "(define list1 (x)     (cons x '()))"
                     , "(define list2 (x y)   (cons x (list1 y)))"
                     , "(define list3 (x y z) (cons x (list2 y z)))"
                     , ";  predefined uScheme functions 101b "
                     , "(define append (xs ys)"
                     , "  (if (null? xs)"
                     , "     ys"
                     , "     (cons (car xs) (append (cdr xs) ys))))"
                     , ";  predefined uScheme functions 102b "
                     ...
                     ,
";definitions of predefined uScheme functions [[and]], [[or]], and [[not]] 98c "
     ...
     "(define alist-first-attribute (alist) (alist-pair-attribute (car alist)))"
                     ...
                     , "      (find k (cdr alist)))))"
                     , ";  predefined uScheme functions 127a "
                     ,
    "(define o (f g) (lambda (x) (f (g x))))          ; ((o f g) x) = (f (g x))"
                     , ";  predefined uScheme functions 128c "
                     , "(define curry   (f) (lambda (x) (lambda (y) (f x y))))"
                     , "(define uncurry (f) (lambda (x y) ((f x) y)))"
                     , ";  predefined uScheme functions 131 "
                     , "(define filter (p? xs)"
                     , "  (if (null? xs)"
                     , "    '()"
                     , "    (if (p? (car xs))"
                     ...
                     , "      (filter p? (cdr xs)))))"
                     , ";  predefined uScheme functions 132a "
                     , "(define map (f xs)"
                     , "  (if (null? xs)"
                     , "    '()"
                     ...
                     , ";  predefined uScheme functions 132c "
                     , "(define exists? (p? xs)"
                     , "  (if (null? xs)"
                     , "    #f"
                     , "    (if (p? (car xs)) "
                     ...
                     , "      (all? p? (cdr xs))"
                     , "      #f)))"
                     , ";  predefined uScheme functions 133b "
                     ...
                     , "                        (val left-square   91)"
                     , "                        (val right-square  93)"
                     , ";  predefined uScheme functions S310f "
                     , "(define <= (x y) (not (> x y)))"
                     , "(define >= (x y) (not (< x y)))"
                     , "(define != (x y) (not (= x y)))"
                     , ";  predefined uScheme functions S311a "
                     , "(define max (x y) (if (> x y) x y))"
                     , "(define min (x y) (if (< x y) x y))"
                     , ";  predefined uScheme functions S311b "
...
(*****************************************************************)
(*                                                               *)
(*   CODE THAT LOOKS AT COMMAND-LINE ARGUMENTS AND CALLS [[RUNAS]] TO RUN THE
INTERPRETER *)
(*                                                               *)
(*****************************************************************)
