...
void pack(quantized q)
{
    ...
    word = Bitpack_news(word, 6, 8, q.d);
    word = Bitpack_news(word, 6, 14, q.c);
    word = Bitpack_news(word, 6, 20, q.b);
    word = Bitpack_newu(word, 6, 26, q.a);
    printBits(word);
}
...
quantized unpack(uint32_t word)
{
    ...
    q.d = Bitpack_gets(word, 6, 8);
    q.c = Bitpack_gets(word, 6, 14);
    q.b= Bitpack_gets(word, 6, 20);
    q.a = Bitpack_getu(word, 6, 26);
    return q;
}
